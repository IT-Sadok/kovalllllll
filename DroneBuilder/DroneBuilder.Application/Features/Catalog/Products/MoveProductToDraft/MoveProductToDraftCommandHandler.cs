...
public sealed class MoveProductToDraftCommandHandler(IProductRepository repository)
    : ICommandHandler<MoveProductToDraftCommand, ProductModel>
{
    public async Task<Result<ProductModel>> ExecuteCommandAsync(
        MoveProductToDraftCommand command,
...
public sealed record MoveProductToDraftCommand(Guid ProductId);
