...
public sealed class GetAdminComponentTypeQueryHandler(ICatalogMetadataRepository repository)
    : IQueryHandler<GetAdminComponentTypeQuery, ComponentTypeDetailsModel>
{
    public async Task<Result<ComponentTypeDetailsModel>> ExecuteAsync(
        GetAdminComponentTypeQuery query,
...
public sealed record GetAdminComponentTypeQuery(Guid ComponentTypeId);
