...
namespace DroneBuilder.Application.Features.Inventory.AddQuantityToWarehouseItem;
    ...
    : ICommandHandler<AddQuantityToWarehouseItemCommand, WarehouseItemModel>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            warehouseItem.AddStock(command.Model.QuantityToAdd);
        }
        catch (ArgumentOutOfRangeException exception)
        {
            return Result.Fail<WarehouseItemModel>(new BadRequestError(exception.Message));
        }
